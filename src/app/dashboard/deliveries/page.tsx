  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-5">
      <div className="flex items-center justify-between">
        ...
        {canCreate && <button onClick={() => setModal(true)} className="btn-primary"><Plus size={16} /> Ambil Stok</button>}
      </div>
            ...
            <form onSubmit={e => { e.preventDefault(); if (!items.length) return toast.error('Tambahkan minimal 1 item'); createMutation.mutate({ items, notes }) }} className="p-5 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={createMutation.isPending} className="btn-primary flex-1 justify-center">
                  {createMutation.isPending && <Loader2 size={14} className="animate-spin" />} Submit & Ambil Stok
                </button>
              </div>
            </form>
          </div>
